use smallvec_wrapper::{OneOrMore, XXLargeVec};

use crate::types::CompressionMap;

use super::{EncodeError, Record, ANCOUNT_OFFSET, MESSAGE_HEADER_SIZE};

const BITS: u16 =
  (1 << 15) // Response set to true
  | (1 << 10); // Authoritative set to true

pub(crate) struct Answer {
  id: u16,
  records: OneOrMore<Record>,
}


impl Answer {
  pub(crate) fn new(id: u16, records: OneOrMore<Record>) -> Self {
    Self { id, records }
  }

  pub(crate) fn encode(&self) -> Result<XXLargeVec<u8>, EncodeError> {
    let mut hbuf = [0u8; MESSAGE_HEADER_SIZE];
    hbuf[0..2].copy_from_slice(&self.id.to_be_bytes());
    hbuf[2..4].copy_from_slice(&BITS.to_be_bytes());
    hbuf[ANCOUNT_OFFSET..ANCOUNT_OFFSET + 2].copy_from_slice(&(self.records.len() as u16).to_be_bytes());
    let mut cmap = CompressionMap::new();
    let mut buf = XXLargeVec::with_capacity(self.encoded_len(&mut cmap));
    buf.extend_from_slice(&hbuf);
    let mut off = MESSAGE_HEADER_SIZE;

    for ans in self.records.iter() {
      off = ans.encode(&mut buf, off, &mut cmap)?;
    }

    buf.truncate(off);
    Ok(buf)
  }

  pub(super) fn encoded_len(&self, cmap: &mut CompressionMap) -> usize {
    let mut l = MESSAGE_HEADER_SIZE;

    for ans in self.records.iter() {
      l += ans.encoded_len(cmap);
    }

    l
  }
}

#[test]
fn t() {
  let x = 0;
  let x1 = x | (1 << 15);
  let x2 = x1 | (1 << 10);
  println!("{x}");
  println!("{}", x2);
}