...
/// on cancellation.
pub async fn query<N>(params: QueryParam<'_>, entry_tx: Sender<ServiceEntry>) -> io::Result<()>
where
  N: Net,
{
...
pub async fn lookup<N>(service: Label<'_>, tx: Sender<ServiceEntry>) -> io::Result<()>
where
  N: Net,
{
  query::<N>(QueryParam::new(service), tx).await
}
